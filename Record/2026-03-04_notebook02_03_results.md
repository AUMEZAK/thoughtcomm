...
## Step 1 実行結果 (2026-03-04)

**コミット**: `6908179` (修正) → `d8bb718` (結果push)

### AE訓練結果（安定化後）
| メトリクス | Step 1以前（爆発） | Step 1以後（安定化） |
|---|---|---|
| rec_loss | 13,807 | **0.0125** |
| jac_loss | 0.025 | 0.0026 |
| total_loss | 13,807 | **0.0126** |
| 訓練曲線 | epoch 150で10^7に爆発 | **安定に収束、爆発なし** |
| B行列スパーシティ | 1.6% | **89.9%** |
| 非ゼロ要素 | 3,095,310 | **318,309** |

### B行列 alpha分布（変化なし）
| alpha | Step 1以前 | Step 1以後 |
|---|---|---|
| alpha=0 | 0 | 0 |
| alpha=1 | 0 | 0 |
| alpha=2 | 0 | 0 |
| alpha=3 | 1024 (100%) | **1024 (100%)** |

### 分析
- **成功**: 訓練安定化は完全に成功。rec_lossが正常収束、スパーシティ大幅改善
- **未解決**: alpha分布が全く変わらない。全1024次元がalpha=3のまま
- **原因**: スパーシティ89.9%でも、残りの10.1%非ゼロが3エージェントに均等に分布
  - 各潜在次元あたり平均311個の非ゼロ → 各エージェントブロックに約104個
  - 1つのエージェントブロック(1024行)が全ゼロになる確率は実質ゼロ
- **根本問題**: L1は要素単位のスパーシティしか誘導できない。ブロック単位のスパーシティ（あるエージェントのブロック全体が0）にはGroup Lasso (L2,1ノルム)が必要
- **L1 weight引き上げは無意味**: スパーシティを99%にしても、残りがランダム分布する限りalpha=3のまま

### 結論
jacobian_l1_weight引き上げ（当初のStep 2）は効果なしと判断。
代わりにGroup Lasso (L2,1ノルム) への変更を次のステップとする。

---

## 修正プラン: Group Lasso導入 (Step 2)

### 目的
L1正則化をGroup Lasso (L2,1ノルム)に変更し、エージェントブロック単位のスパーシティを誘導する。
これによりalpha<3の次元（特定エージェントに無関係な次元）が出現することを期待。

### L1 vs Group Lasso
- **L1**: `penalty = Σ|J[i,j]|` → 各要素を個別に0に押す → まばらに点々とスパース
- **Group Lasso**: `penalty = Σ_j Σ_k ||J[k*H:(k+1)*H, j]||_2` → エージェントブロック単位で0に押す → ブロックスパース

### 修正ファイル

#### 1. `training/jacobian_utils.py`
- `stochastic_jacobian_group_l1()` 関数を新規追加
- Jacobian J: (batch, n_h, n_z) → (batch, num_agents, hidden_size, n_z) にreshape
- 各(agent, latent dim)ごとにL2ノルム → agent方向に合計

#### 2. `training/train_autoencoder.py`
- configのフラグに応じて L1 / Group L1 を切り替え

#### 3. `configs/config.py`
- `jacobian_reg_type: str = "group_l1"` パラメータ追加
